...
priority_queue< pair<int, Task>, vector< pair<int, Task>>, greater < pair<int, Task>>> ready;
...
int main(){
    ...
    ready.push(make_pair(wait.top().second.burst, wait.top().second));
    wait.pop();
    ...
    while(!wait.empty() || !ready.empty()){
        ...
        if(ready.top().second.burst<current.burst-(time-start)){
            temp = current;
